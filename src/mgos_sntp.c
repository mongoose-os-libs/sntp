...
static void mgos_sntp_retry_timer_cb(void *user_data) {
  ...
  if (s_state.idx == 0 && s_state.local_server[0] != '\0' && mgos_sys_config_get_sntp_follow_dhcp_ntp()) {
...
out:
  if (ip_info.ntp.sin_addr.s_addr != 0 && mgos_sys_config_get_sntp_follow_dhcp_ntp()) {
